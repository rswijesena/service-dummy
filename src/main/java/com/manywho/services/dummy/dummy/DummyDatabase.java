...
public class DummyDatabase implements ReadOnlyDatabase<ApplicationConfiguration, Dummy> {
    @Override
    public Dummy find(ApplicationConfiguration configuration, Command command, String id) {
        return new Dummy("123", "Jonjo", 23);
    }

    @Override
    public List<Dummy> findAll(ApplicationConfiguration configuration, Command command, ListFilter filter) {
